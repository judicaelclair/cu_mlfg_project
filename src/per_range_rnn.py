...
import per_range_util as util
from conv1d_vae import VAE
    
class PerRangeRNN(nn.Module):
    # module = nn.RNN | nn.GRU | nn.LSTM
    def __init__(self, input_size, output_size, hidden_rnn_size = 32, num_layers = 3, module = nn.RNN, dropout = 0.2, bidirectional = True, double_hidden_fc_size = True, vae = False):
        super(PerRangeRNN, self).__init__()
    ...
    def forward(self, x):
        ...
        for i in range(batch_size):
            fwd_out.append(unpacked_out[i, unpacked_lengths[i]-1, :self.hidden_rnn_size])
...
if __name__ == "__main__":
    ...
    model = PerRangeRNN(train_dataset.dataset.in_len, train_dataset.dataset.out_len)
    print(model)
